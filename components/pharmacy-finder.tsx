...
export default function PharmacyFinder() {
    ...
    const fetchNearbyPharmacies = async (lat: number, lon: number, searchRadius: number = radius) => {
        ...
        try {
            const query = `[out:json];nwr["amenity"="pharmacy"](around:${searchRadius},${lat},${lon});out center;`;
            ...
            if (!data.elements || data.elements.length === 0) {
                setPharmacies([]);
                setError(`No nearby pharmacies found within ${searchRadius / 1000} km.`);
            } else {
                const results = data.elements
                    ...
                    .map((el: any) => {
                        ...
                        const placeLoc = { latitude: elLat, longitude: elLon };
                        return {
                            id: el.id,
